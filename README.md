...
7. Once the SQL parsing is tightened up, it's still vulnerable to malicious users going around your front end and doing custom queries on tables they have write permission on.  As such, it's not suitable for applications with public/random users without, for example, disabling delete and update queries.
